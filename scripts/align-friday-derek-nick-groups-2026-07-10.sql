-- =============================================================================
-- RAW Friday 2026-07-10 — align 6 wrestlers to coach groups
--
--   Derek (3): Cameron Steiner, Gabriel Jager, Jahiem Skyers
--   Nick  (3): Grady Brewer, Rahiem Skyers, Aidan Hamilton
--
-- Prerequisite: each wrestler has signed up (Guild account + waiver).
-- Run STEP A first, then STEP B. Payment / school invoice handled separately.
-- =============================================================================

-- -----------------------------------------------------------------------------
-- STEP A — Discover Friday sessions + wrestler rows (read-only)
-- -----------------------------------------------------------------------------
SELECT
  s.id AS session_id,
...
WHERE (timezone('America/New_York', s.scheduled_datetime))::date = DATE '2026-07-10'
  AND (
...
DO $$
DECLARE
  session_day date := DATE '2026-07-10';
...
BEGIN
  ...
  IF v_derek_session IS NULL THEN
    RAISE EXCEPTION 'No Derek session on %. Run STEP A and fix session_day or coach filter.', session_day;
  END IF;
  IF v_nick_session IS NULL THEN
    RAISE EXCEPTION 'No Nick session on %. Run STEP A and fix session_day or coach filter.', session_day;
  END IF;
  ...
  IF v_cam IS NULL OR v_gabriel IS NULL OR v_jahiem IS NULL
    ...
    RAISE EXCEPTION 'Missing user for one of the six emails — wait until all accounts exist, then rerun.';
  END IF;
