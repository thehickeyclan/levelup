-- Unify market listing activity posts (collection, sell, trade) under market_listing_published

UPDATE public.activity_posts
SET trigger_type = 'market_listing_published'
WHERE trigger_type = 'market_collection_listed';

DROP INDEX IF EXISTS public.idx_activity_posts_market_collection_listed;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_activity_posts_market_listing_published
  ON public.activity_posts (market_listing_id)
  WHERE trigger_type = 'market_listing_published' AND market_listing_id IS NOT NULL;

-- Backfill sell/trade listings already live with photos
...
SELECT
  ...
  COALESCE(
    (
      SELECT yw.profile_public IS NOT FALSE
      FROM public.youth_wrestlers yw
      WHERE yw.parent_id = ml.seller_id
      ORDER BY yw.created_at ASC
      LIMIT 1
...
WHERE ml.listing_type IN ('sell', 'trade')
  ...
  AND NOT EXISTS (
    ...
    WHERE ap.market_listing_id = ml.id
      AND ap.trigger_type = 'market_listing_published'
  );
